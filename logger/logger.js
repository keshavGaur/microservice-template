...
const accessLogStream = logConf.accessLogsRedirectToStdout
  ? process.stdout
  : rfs(logConf.accessLogFileName, accessLogConf);
const bunyanStream = logConf.apiLogsRedirectToStdout
  ? process.stdout
  : new bunyan.RotatingFileStream(appLogConf);
...
Logger.prototype.createLogSessionForRequest =
  function createLogSessionForRequest(req, res, next) {
    loggerSession.bindEmitter(req);
    loggerSession.run(() => {
      loggerSession.set(
        "reqId",
        req && req.headers
          ? req.headers[constants.X_REQUEST_ID_HEADER]
          : undefined
      );
      next();
    });
